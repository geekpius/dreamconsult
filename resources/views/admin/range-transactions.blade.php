<table id="example" class="table table-striped table-borderless">
    <thead>
        <tr><th colspan="9" class="text-primary"><strong>TRANSACTIONS - From {{ \Carbon\Carbon::parse($start)->format('D d-M-Y') }} To {{ \Carbon\Carbon::parse($end)->format('D d-M-Y') }}</strong></th></tr>
        <tr>
            <th>Rec#</th> 
            <th>Pay Date</th> 
            <th>Client ID</th>                            
...
var table = $('#example').DataTable( {
    ...
    buttons: [
            ...
            },
            {
                extend: 'pdfHtml5',
                title: 'DATE RANGE TRANSACTIONS',
                messageTop: "TRANSACTIONS - From {{ \Carbon\Carbon::parse($start)->format('D d-M-Y') }} To {{ \Carbon\Carbon::parse($end)->format('D d-M-Y') }}",
                exportOptions: {
                    columns: [ 0, 1, 2, 3, 4, 5, 6, 7 ]
                }
            },
