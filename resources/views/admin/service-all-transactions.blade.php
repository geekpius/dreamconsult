<table id="example" class="table table-striped table-borderless">
    <thead>
        <tr><th colspan="9" class="text-primary"><strong>ALL SERVICE TRANSACTIONS AS AT {{ \Carbon\Carbon::now()->format('d-M-Y') }}</strong></th></tr>
        <tr>
            <th>Rec#</th> 
            <th>Pay Date</th>                        
            <th>Service Name</th>  
...
var table = $('#example').DataTable( {
    ...
    buttons: [
            {
                extend: 'excelHtml5',
                title: 'ALL SERVICE TRANSACTIONS',
                exportOptions: {
            ...
            },
            {
                extend: 'pdfHtml5',
                title: 'ALL SERVICE TRANSACTIONS',
                messageTop: "ALL TRANSACTIONS AS AT {{ \Carbon\Carbon::now()->format('d-M-Y') }}",
                exportOptions: {
                    columns: [ 0, 1, 2, 3, 4, 5, 6, 7 ]
                }
            },
            {
                extend: 'print',
                title: 'ALL SERVICE TRANSACTIONS',
                messageTop: "ALL SERVICE TRANSACTIONS AS AT {{ \Carbon\Carbon::now()->format('d-M-Y') }}",
                exportOptions: {
                    columns: [ 0, 1, 2, 3, 4, 5, 6, 7 ]
                }
            },
