<table id="example" class="table table-striped table-borderless">
    <thead>
        <tr><th colspan="9" class="text-primary"><strong>WEEKLY SERVICE TRANSACTIONS - From {{ \Carbon\Carbon::now()->startOfWeek()->format('D d-M-Y') }} To {{ \Carbon\Carbon::now()->endOfWeek()->format('D d-M-Y') }}</strong></th></tr>
        <tr>
            <th>Rec#</th> 
            <th>Pay Date</th>                        
            <th>Service Name</th>  
...
var table = $('#example').DataTable( {
    ...
    buttons: [
            {
                extend: 'excelHtml5',
                title: 'WEEKLY SERVICE TRANSACTIONS',
                exportOptions: {
            ...
            },
            {
                extend: 'pdfHtml5',
                title: 'WEEKLY SERVICE TRANSACTIONS',
                messageTop: "WEEKLY TRANSACTIONS - From {{ \Carbon\Carbon::now()->startOfWeek()->format('D d-M-Y') }} To {{ \Carbon\Carbon::now()->endOfWeek()->format('D d-M-Y') }}",
                exportOptions: {
                    columns: [ 0, 1, 2, 3, 4, 5, 6, 7 ]
                }
            },
            {
                extend: 'print',
                title: 'WEEKLY SERVICE TRANSACTIONS',
                messageTop: "WEEKLY SERVICE TRANSACTIONS - From {{ \Carbon\Carbon::now()->startOfWeek()->format('D d-M-Y') }} To {{ \Carbon\Carbon::now()->endOfWeek()->format('D d-M-Y') }}",
                exportOptions: {
                    columns: [ 0, 1, 2, 3, 4, 5, 6, 7 ]
                }
            },
